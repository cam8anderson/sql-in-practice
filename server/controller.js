...
module.exports = {
  ...
  getAllClients: (req, res) => {
    sequelize
      .query(
        `SELECT * FROM cc_users INNER JOIN cc_clients ON cc_users.user_id = cc_clients.user_id;`
      )
      .then((dbRes) => res.status(200).send(dbRes[0]));
  },
  ...
  getPendingAppointments: (req, res) => {
    sequelize
      .query(
        `select * from cc_appointments where approved = false and completed = false order by date desc;`
      )
      .then((dbRes) => res.status(200).send(dbRes[0]));
  },
